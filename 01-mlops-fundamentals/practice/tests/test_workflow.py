...
@pytest.fixture
def api():
    assert importlib.util.find_spec("taxi_duration"), "Implement the taxi_duration package"
...
def labeled(month=1):
    ...
    return pd.DataFrame({
        ...
        "lpep_dropoff_datetime": start + pd.to_timedelta([2, 4, 2, 4, 2, 4, 2, 4], unit="min"),
...
def test_invalid_zone_is_not_silently_truncated(api, bad):
    ...
    with pytest.raises(ValueError):
        data.prepare_features(pd.DataFrame({"PULocationID": [bad], "DOLocationID": [2]}))
...
def run_cli(tmp_path, *args):
    ...
    return subprocess.run([sys.executable, "-m", "taxi_duration.cli", *map(str, args)],
...
def test_cli_train_and_predict_in_another_directory(api, tmp_path):
    ...
    inputs = pd.DataFrame({"ride_id": ["ride-b", "ride-a"], "PULocationID": [2, 1], "DOLocationID": [3, 3]})
    inputs.to_csv(tmp_path / "input.csv", index=False)
    result = run_cli(tmp_path, "train", "--train", "train.parquet", "--validation", "validation.parquet", "--output", "artifacts")
    ...
    result = run_cli(tmp_path, "predict", "--model", "artifacts/model.pkl", "--input", "input.csv", "--output", "predictions.csv")
    assert result.returncode == 0, result.stderr
...
def test_cli_rejects_duplicate_ride_ids(api, tmp_path):
    ...
    pd.DataFrame({"ride_id": ["same", "same"], "PULocationID": [1, 2], "DOLocationID": [3, 3]}).to_csv(tmp_path / "input.csv", index=False)
    result = run_cli(tmp_path, "predict", "--model", "model.pkl", "--input", "input.csv", "--output", "out.csv")
    assert result.returncode != 0
    assert not (tmp_path / "out.csv").exists()


def test_data_preparation_is_local_repeatable_and_has_provenance(tmp_path):
    raw = tmp_path / "raw"
    raw.mkdir()
    for month in (1, 2):
        labeled(month).to_parquet(raw / f"green_tripdata_2021-{month:02d}.parquet")
    script = Path(__file__).resolve().parents[1] / "prepare_data.py"
    for name in ("sample-a", "sample-b"):
        result = subprocess.run([sys.executable, str(script), "--raw-dir", str(raw),
                                 "--output", str(tmp_path / name), "--rows", "4"],
                                capture_output=True, text=True)
        assert result.returncode == 0, result.stderr
    a, b = tmp_path / "sample-a", tmp_path / "sample-b"
    pd.testing.assert_frame_equal(pd.read_parquet(a / "train.parquet"), pd.read_parquet(b / "train.parquet"))
    manifest = json.loads((a / "manifest.json").read_text())
    assert manifest["files"]["train.parquet"]["rows"] == 4
    assert len(manifest["files"]["train.parquet"]["source_sha256"]) == 64
    inference = pd.read_csv(a / "inference.csv")
    assert set(inference.columns) == {"ride_id", "PULocationID", "DOLocationID"}
    assert inference.ride_id.is_unique
