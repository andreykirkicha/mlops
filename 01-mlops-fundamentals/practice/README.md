...
- [01-experiment.ipynb](01-experiment.ipynb) — эксперимент с TODO: подготовка данных, обучение и предсказание.
...
Сначала заполните TODO в notebook, затем перенесите написанную логику в пакет.
Упрощённые преобразования notebook не заменяют строгую валидацию пакета.
...
| Этап | Что делаем |
|---|---|
| Эксперимент | Заполняем TODO в notebook по порядку; после завершения — Restart → Run All |
| Реализация | Переносим логику из notebook в три TODO пакета |
...
В notebook сначала заполните TODO 1, 2a/2b и 3: подготовку данных, признаки
и обучение, предсказание. Незавершённые ячейки останавливаются на
`NotImplementedError`. После заполнения проверьте весь notebook с чистым ядром.

Затем перенесите код в пакет: `labeled_cohort` → `prepare_training_data`,
блоки TODO 2a/2b → `train_model`, `predict_requests` → `predict`.
Используйте готовую `prepare_features` вместо `feature_records`; в `predict`
модель и vectorizer берутся из `bundle`. Имена переменных следуют заготовке.
...
Используйте свою реализацию `labeled_cohort` из notebook.
